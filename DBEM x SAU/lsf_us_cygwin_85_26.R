...
split_USA <- split(usa_lsf_cygwin, usa_lsf_cygwin$taxonID)
...
MISTAKES_us_26_lsf <- anti_join(usa_lsf_cygwin, cygwin_RCP26, by = "taxonID")

us_lsf_cygwin_partial <- anti_join(usa_lsf_cygwin, MISTAKES_us_26_lsf) %>% 
...
for(i in 1:length(us_lsf_cygwin_partial$taxonID)){
  taxontemp <-as.character(us_lsf_cygwin_partial$taxonID[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL26/",taxontemp,".csv",sep="")
...
write_csv(USA26_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL26/errspp_cygwin")

##########################################################################
# RCP: 8.5

MISTAKES_us_85_lsf <- anti_join(usa_lsf_cygwin, cygwin_RCP85, by = "taxonID")

us_lsf_cygwin_partial <- anti_join(usa_lsf_cygwin, MISTAKES_us_85_lsf) 
...
for(i in 1:length(us_lsf_cygwin_partial$taxonID)){
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL85/",taxontemp,".csv",sep="")
    write.csv(sau_dbem,filepath,row.names=F)
  }
  else{
    errspp[i] <-  taxontemp
...
write_csv(USA85_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL85/errspp_cygwin")
