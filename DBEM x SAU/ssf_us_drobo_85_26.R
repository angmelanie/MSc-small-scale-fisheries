...
split_USA <- split(usa_ssf_drobo, usa_ssf_drobo$taxonID)
...
USA_eez_cellID <- read.csv(file = "C:/Users/angmel/Documents/firstchapter/Reference tables/eez_cellID.csv") %>% 
  filter(EEZID %in% "848") %>%    # EEZ = USA subarctic Alaska 
  rename(eez = EEZID, INDEX = CellID.)

# upload USA taxa list
usa_ssf_drobo$taxonID
...
for(i in 1:length(usa_ssf_drobo$taxonID)){
  taxontemp <-as.character(usa_ssf_drobo$taxonID[i])
...
USA26_DROBO <- errspp # CHANGE THIS FOR DROBO/CYGWIN
USA26_DROBO <- data.frame(USA26_DROBO) # make data frame format for saving
write_csv(USA26_DROBO, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL26/SSF/errspp")
...
# RUN THESE LINES############################################################!!!!!!*******
USA85_DROBO <- errspp # CHANGE THIS FOR DROBO/CYGWIN
USA85_DROBO <- data.frame(USA85_DROBO) # make data frame format for saving
write_csv(USA85_DROBO, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL85/SSF/errspp")
