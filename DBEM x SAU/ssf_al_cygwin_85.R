# SSF, Alaska, cygwin, RCP: 8.5

# split catch by taxa and store in global env
split_Alaska <- split(alaska_ssf_cygwin, alaska_ssf_cygwin$taxonID)
...
MISTAKES_al_85_ssf <- anti_join(alaska_ssf_cygwin, cygwin_RCP85, by = "taxonID")

alaska_ssf_cygwin_partial <- anti_join(alaska_ssf_cygwin, MISTAKES_al_85_ssf) %>% 
  filter(!taxonID %in% NA)
...
for(i in 1:length(alaska_ssf_cygwin_partial$taxonID)){
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Alaska/GFDL85/SSF/",taxontemp,".csv",sep="")
...
Alaska85_cygwin <- errspp # CHANGE THIS FOR DROBO/CYGWIN
Alaska85_cygwin <- data.frame(Alaska85_cygwin) # make data frame format for saving
write_csv(Alaska85_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Alaska/GFDL85/SSF/errspp85")
