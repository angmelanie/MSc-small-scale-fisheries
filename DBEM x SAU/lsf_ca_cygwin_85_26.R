# Canada, SSF, CYGWIN, 26


# split catch by taxa and store in global env
split_CA <- split(canada_lsf_cygwin, canada_lsf_cygwin$taxonID)
...
Canada_eez_cellID <- read.csv(file = "C:/Users/angmel/Documents/firstchapter/Reference tables/eez_cellID.csv") %>% 
  filter(EEZID %in% "925") %>%    # EEZ = Canada West
  rename(eez = EEZID, INDEX = CellID.)

# upload Canada taxa list
canada_lsf_cygwin$taxonID

# RCP26
cygwin_RCP26 # species i actually modelled
MISTAKES_ca_26_lsf <- anti_join(canada_lsf_cygwin, cygwin_RCP26, by = "taxonID")
canada_lsf_cygwin_partial <- anti_join(canada_lsf_cygwin, MISTAKES_ca_26_lsf) %>% 
  ungroup() %>% 
  filter(!taxonID %in% `800254`) %>% 
  filter(!taxonID %in% `800256`) %>% 
  filter(!taxonID %in% `800253`) %>% 
  filter(!taxonID %in% `800255`)


cygwin_RCP85
MISTAKES_ca_85_lsf <- anti_join(canada_lsf_cygwin, cygwin_RCP85, by = "taxonID")
canada_lsf_cygwin_par85 <- anti_join(canada_lsf_cygwin, MISTAKES_ca_85_lsf) %>% 
  ungroup() %>% 
  filter(!taxonID %in% `800254`)%>% 
  filter(!taxonID %in% `800256`)%>% 
  filter(!taxonID %in% `800253`)%>% 
...
for(i in 1:length(canada_lsf_cygwin_par85$taxonID)){
  taxontemp <-as.character(canada_lsf_cygwin_par85$taxonID[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Canada/GFDL85/",taxontemp,".csv",sep="")
...
Canada85_cygwin <- errspp # CHANGE THIS FOR DROBO/CYGWIN
Canada85_cygwin <- data.frame(Canada85_cygwin) # make data frame format for saving
write_csv(Canada85_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Canada/GFDL85/errspp85cygwin")
