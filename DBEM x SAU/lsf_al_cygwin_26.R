...
MISTAKES_al_26 <- anti_join(alaska_lsf_cygwin, cygwin_RCP26, by = "taxonID")

alaska_lsf_cygwin_partial <- anti_join(alaska_lsf_cygwin, MISTAKES_al_26) %>% 
  filter(!taxonID %in% `604037`) %>% # ADD THESE!!!! Rerun DBEM with these!!!!
...
for(i in 1:length(alaska_lsf_cygwin_partial$taxonID)){
  taxontemp <-as.character(alaska_lsf_cygwin_partial$taxonID[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Alaska/GFDL26/",taxontemp,".csv",sep="")
...
write_csv(Alaska26_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Alaska/GFDL26/errspp26")
