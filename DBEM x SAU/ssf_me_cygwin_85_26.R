...
split_me <- split(me_ssf_cygwin, me_ssf_cygwin$taxonID)
...
MISTAKES_me_26_ssf <- anti_join(me_ssf_cygwin, cygwin_RCP26, by = "taxonID")

me_ssf_cygwin_partial <- anti_join(me_ssf_cygwin, MISTAKES_me_26_ssf) %>% 
  ungroup() %>% 
...
for(i in 1:length(me_ssf_cygwin_partial$taxonID)){
  taxontemp <-as.character(me_ssf_cygwin_partial$taxonID[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Mexico/GFDL26/SSF/",taxontemp,".csv",sep="")
...
write_csv(Mexico26_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Mexico/GFDL26/SSF/errspp_cygwin")
...
# OOOOOOOOOOPS YOU DID NOT MODEL THESE BUT YOU SHOULD HAVE!!!!!!!
# remove FOR NOW, accumulate list for after!!!!!!!!!!!!!!!!!!!!!!
MISTAKES_me_85_ssf <- anti_join(me_ssf_cygwin, cygwin_RCP85, by = "taxonID")

me_ssf_cygwin_partial <- anti_join(me_ssf_cygwin, MISTAKES_me_85_ssf) 
...
for(i in 1:length(me_ssf_cygwin_partial$taxonID)){
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Mexico/GFDL85/SSF/",taxontemp,".csv",sep="")
    write.csv(sau_dbem,filepath,row.names=F)
  }
  else{
    errspp[i] <-  taxontemp
...
write_csv(Mexico85_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Mexico/GFDL85/SSF/errspp_cygwin")
