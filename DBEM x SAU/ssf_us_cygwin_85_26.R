...
split_USA <- split(usa_ssf_cygwin, usa_ssf_cygwin$taxonID)
...
USA_eez_cellID <- read.csv(file = "C:/Users/angmel/Documents/firstchapter/Reference tables/eez_cellID.csv") %>% 
  filter(EEZID %in% "848") %>%    # EEZ = USA west
  rename(eez = EEZID, INDEX = CellID.)

# upload USA taxa list
usa_ssf_cygwin$taxonID
...
MISTAKES_us_26_ssf <- anti_join(usa_ssf_cygwin, cygwin_RCP26, by = "taxonID")

us_ssf_cygwin_partial <- anti_join(usa_ssf_cygwin, MISTAKES_us_26_ssf) %>% 
  filter(!taxonID %in% `604032`) %>% 
  filter(!taxonID %in% `608838`) %>% 
  filter(!taxonID %in% `603962`) %>% 
...
for(i in 1:length(us_ssf_cygwin_partial$taxonID)){
  taxontemp <-as.character(us_ssf_cygwin_partial$taxonID[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL26/SSF/",taxontemp,".csv",sep="")
...
USA26_cygwin <- errspp # CHANGE THIS FOR DROBO/CYGWIN
USA26_cygwin <- data.frame(USA26_cygwin) # make data frame format for saving
write_csv(USA26_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL26/SSF/errspp_cygwin")

##########################################################################
# RCP: 8.5

MISTAKES_us_85_ssf <- anti_join(usa_ssf_cygwin, cygwin_RCP85, by = "taxonID")

us_ssf_cygwin_partial <- anti_join(usa_ssf_cygwin, MISTAKES_us_85_ssf) 
...
for(i in 1:length(us_ssf_cygwin_partial$taxonID)){
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL85/SSF/",taxontemp,".csv",sep="")
    write.csv(sau_dbem,filepath,row.names=F)
  }
  else{
    errspp[i] <-  taxontemp
...
# RUN THESE LINES############################################################!!!!!!*******
USA85_cygwin <- errspp # CHANGE THIS FOR DROBO/CYGWIN
USA85_cygwin <- data.frame(USA85_cygwin) # make data frame format for saving
write_csv(USA85_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/USA/GFDL85/SSF/errspp_cygwin")
