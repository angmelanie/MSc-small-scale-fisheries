# SSF, Alaska, cygwin, RCP: 2.6

# split catch by taxa and store in global env
# split_Alaska <- split(alaska_lsf_cygwin, alaska_lsf_cygwin$taxonID)
# list2env(split_Alaska, envir = .GlobalEnv)
# 
# cygwin_RCP26 <- read.csv("C:/Users/angmel/Documents/MSc-small-scale-fisheries/DBEM x SAU/cygwin_RCP26.csv", header = FALSE) %>% 
#   rename(taxonID = V1)

# OOOOOOOOOOPS YOU DID NOT MODEL THESE BUT YOU SHOULD HAVE!!!!!!!
# remove FOR NOW, accumulate list for after!!!!!!!!!!!!!!!!!!!!!!
MISTAKES_al_26_ssf <- anti_join(alaska_ssf_cygwin, cygwin_RCP26, by = "taxonID")

alaska_ssf_cygwin_partial <- anti_join(alaska_ssf_cygwin, MISTAKES_al_26_ssf) %>% 
  filter(!taxonID %in% `604032`) %>% # ADD THESE!!!! Rerun DBEM with these!!!!
  filter(!taxonID %in% `604122`) %>% 
  filter(!taxonID %in% `603962`) %>% 
  filter(!taxonID %in% `604037`) %>% 
  filter(!taxonID %in% `800138`)
View(alaska_lsf_cygwin_partial)

errspp <- numeric()

for(i in 1:length(alaska_ssf_cygwin_partial$taxonID)){
  taxontemp <-as.character(alaska_ssf_cygwin_partial$taxonID[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Alaska/GFDL26/SSF/",taxontemp,".csv",sep="")
...
Alaska26_cygwin <- errspp # CHANGE THIS FOR DROBO/CYGWIN
Alaska26_cygwin <- data.frame(Alaska26_cygwin) # make data frame format for saving
write_csv(Alaska26_cygwin, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/Alaska/GFDL26/errspp26ssf")
