...
# 3. error log: assign to refresh, reassign and save variable
...
Mexico_eez_cellID <- read.csv(file = "C:/Users/angmel/Documents/firstchapter/Reference tables/eez_cellID.csv") %>% 
  filter(EEZID %in% "945") %>%    # EEZ = Mexico PAcific
...
for(i in 1:length(Mexico_cygwin$V1)){
  taxontemp <-as.character(Mexico_cygwin$V1[i])
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/firstchapter/SAU_DBEM_RESULTS/Mexico/GFDL26/",taxontemp,".csv",sep="")
...
for(i in 1:length(Mexico_cygwin$V1)){
  ...
  if (sum(dbem[1,], na.rm = TRUE) > 0) {
    ...
    filepath<-paste("C:/Users/angmel/Documents/firstchapter/SAU_DBEM_RESULTS/Mexico/GFDL85/",taxontemp,".csv",sep="")
    write.csv(sau_dbem,filepath,row.names=F)
  }
  else{
    errspp[i] <-  taxontemp
...
Mexico85_cygwin <- errspp 


# save error files
write.csv(Mexico26_DROBO, "MEXICO26DROBO")
write.csv(Mexico85_DROBO, "MEXICO85DROBO")

write.csv(Mexico26_cygwin, "MEXICO26CYGWIN")
write.csv(Mexico85_cygwin, "MEXICO85CYGWIN")
