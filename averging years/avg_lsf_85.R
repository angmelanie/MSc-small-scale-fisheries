...
# LSf - 8.5
path <- "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/LSF/85/"

filename <- dir(path, pattern = ".csv")
filepath <- paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/LSF/85/", filename, sep = "")
...
for (i in 1:length(filepath)){
  ...
  if (sum(sau_dbem[2,], na.rm = TRUE) > 0) {
    ...
    # final result
    newfilepath <- paste("C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/LSF/85/avg/", filename[i], "_avg.csv", sep = "")
...
df<-data.frame(errspp = unlist(errspp))

write_csv(df, "C:/Users/angmel/Documents/MSc-small-scale-fisheries/SAU_DBEM_RESULTS/LSF/85/avg/lsf_85.csv")
